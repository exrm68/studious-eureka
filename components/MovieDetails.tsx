      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-[320px] w-full">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22 }} className="mb-7">
          {isWatch ? (
            <>
              ...
              <p className="text-gray-400 text-sm leading-relaxed">এখানেই play হবে — কোথাও যেতে হবে না! 🎬</p>
            </>
          ) : (
            <>
              <div className="flex items-center justify-center gap-2 mb-1.5">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.28 }}
          ...
          <button onClick={onConfirm}
            ...
            style={{
              background: isWatch ? 'linear-gradient(135deg,#FFD700,#FFA500)' : 'linear-gradient(135deg,#22c55e,#15803d)',
              boxShadow: isWatch ? '0 8px 28px rgba(255,200,0,0.4)' : '0 8px 28px rgba(34,197,94,0.38)',
              color: isWatch ? '#000' : '#fff',
            }}>
            {isWatch ? <><Play size={17} fill="currentColor" />হ্যাঁ, এখনই দেখুন</> : <><Download size={17} />হ্যাঁ, Download করুন</>}
          </button>
        </motion.div>
      </div>
    </motion.div>
...
const MovieDetails: React.FC<MovieDetailsProps> = ({ movie, onClose, botUsername, channelLink, appSettings }) => {
  ...
  // ── 🎬 In-App Video Player ──
  const [playerState, setPlayerState] = useState<{ open: boolean; code: string; title: string }>({ open: false, code: '', title: '' });
  const openPlayer = useCallback((code: string, title: string) => setPlayerState({ open: true, code, title }), []);
  const closePlayer = useCallback(() => setPlayerState({ open: false, code: '', title: '' }), []);
  ...
  const handleWatch = (code: string) => {
    // 🎬 Mini App এ In-App Player খুলবে — Bot এ যাবে না
    runMovieAction('watch', watchAdCount, () => openPlayer(code, movie.title));
  };
  ...
  const handleDownload = (downloadCode?: string, downloadLink?: string, fallbackCode?: string) => {
    ...
    runMovieAction('download', downloadAdCount, execute);
  };
  ...
  const handleEpWatch = (ep: { title: string; number: number; id: string; telegramCode: string; watchAdCount?: number }) => {
    ...
    // 🎬 Episode in-app player
    runEpisodeAction(ep, epAdCount, () => openPlayer(ep.telegramCode, `${movie.title} — Ep ${ep.number}`), 'watch');
  };
      ...
      <AnimatePresence>
        {playerState.open && (
          <VideoPlayer
            ...
            thumbnail={movie.detailBanner || movie.thumbnail}
            onClose={closePlayer}
          />
        )}
      </AnimatePresence>
